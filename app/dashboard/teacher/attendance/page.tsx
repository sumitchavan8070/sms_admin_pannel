...
export default function TeacherAttendanceCalendar() {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        ...
        if (!res || res.status !== 1) {
          console.warn("Attendance fetch failed or returned unexpected structure")
          return
        }

        const rawData: AttendanceRecord[] = res.result.map((r: any) => ({
          date: new Date(r.date),
          status: r.status,
          checkIn: r.checkInTime,
          checkOut: r.checkOutTime,
          shift: r.checkInTime && r.checkOutTime ? "GS" : undefined,
        }))
  ...
  const renderDay = (day: Date) => {
    ...
    return (
      ...
      >
        <div className="font-semibold text-xs leading-none">{format(day, "d")}</div>
        <div className="text-xl font-bold mt-1 leading-none">{status !== "none" ? statusMap[status] : ""}</div>
        {record?.shift && <div className="text-[10px] mt-1 leading-none font-medium">{record.shift}</div>}
      </div>
    )
  }
      ...
      <Card className="w-full">
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-7 gap-2 mb-4 text-center">
            {DAY_NAMES.map((d) => (
              <div key={d} className="text-xs font-semibold text-gray-500 dark:text-gray-400">
                {d}
              </div>
            ))}
          </div>
            ...
            {Object.entries(statusDescriptions).map(([key, value]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-1">
                  <span className={cn("inline-block w-4 h-4 rounded-full", colorMap[statusKey])}></span>
                  <span>{statusMap[statusKey]} - {value}</span>
                </div>
              )
            })}
          </div>
          ...
          {selectedRecord ? (
            <div className="space-y-4 text-sm">
              ...
              {selectedRecord.shift && (
                <>
                  <div>
                    <strong className="block text-xs uppercase text-gray-500 dark:text-gray-400">Shift</strong>
                    <div>{selectedRecord.shift}</div>
                  </div>
                  <div>
                    <strong className="block text-xs uppercase text-gray-500 dark:text-gray-400">Check-in / Check-out</strong>
                    <div>{selectedRecord.checkIn} - {selectedRecord.checkOut}</div>
                  </div>
                </>
              )}
            </div>
